...
p_load(rio, ggplot2, shiny, readxl, haven, RColorBrewer, plotly, stringr, readr)

#==== define function ====
gg_shiny <- function( dataset = NA ) {
#====🏃 UI ====
  #==== UI-fluidePage ====
  ui <- fluidPage(
    headerPanel("ggplot GUI"),
    #==== ⭐️ UI-fluidePage-sidebarPanel ====
    ...
    ),#==== close UI-fluidePage-sidebarPanel ⭐️ ====
...
#####################################################################
#==== ⭐️ UI-fluidePage-MainPanel ====
    mainPanel(width = 6,
      #==== ⭐️ ⭐️ UI-fluidePage-MainPanel-tabsetPanel ====
      tabsetPanel(
        type = "tabs",
        #==== 📌  Data Upload table ====
        ...
        tabPanel("ggplot",
                 mainPanel(
                   ...
                   tags$hr(),
                   tags$hr(),
                   downloadButton("download_plot_PPT",
                                  "Download figure"))
                ),
        #==== 📌  Plotly ====
        ...
        id = "tabs"
      )# close-UI-fluidePage-MainPanel-tabsetPanel
    ),# close-UI-fluidePage-MainPanel
...
#==== 📌 download batten code ====
  output$download_plot_PPT <- downloadHandler(
      filename <- function() {
        paste("Figure_", Sys.time(), ".pptx", sep = "")
      },
      content <- function(file) {
        ...
        doc <- addPlot(doc,function()
          print(p, newpage = FALSE), vector.graphic = TRUE)
        writeDoc(doc, file = file)
      },
      contentType = "application/PPTX" # MIME type of the image
  )
